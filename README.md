...
대부분의 호스팅은 `PORT`를 자동으로 넣어주므로 직접 설정하지 않아도 됩니다. 업로드된 슬라이드는 서버 파일 시스템의 `UPLOAD_DIR`에 저장되고, 방 코드/플레이어/슬라이드 목록은 `DATA_DIR`에 저장됩니다. 무료/임시 파일 시스템을 쓰는 플랫폼에서는 재시작 후 업로드 파일이나 저장된 방 정보가 사라질 수 있으니, 실제 행사에서 오래 보관해야 하면 영구 디스크가 있는 플랜이나 외부 스토리지를 사용하세요.
...
9. 시상식 화면의 `시상식 끝내고 처음으로`를 누르면 같은 방과 슬라이드를 유지한 채 첫 슬라이드로 돌아가고 점수는 초기화됩니다.

## 행사장 재사용 흐름

- 시작 화면의 `저장된 방` 목록에서 기존 방을 `호스트` 또는 `뷰어`로 바로 열 수 있습니다.
- 호스트 화면의 `방 저장`을 누르면 현재 슬라이드, 참가자, 점수 상태가 저장됩니다.
- 호스트 화면의 `뷰어 화면`, 뷰어 화면의 `호스트 화면` 버튼으로 같은 탭에서 역할만 바꿀 수 있습니다.
- 아이패드에서 호스트 화면을 나가거나 다른 방에 들어가도 서버가 살아 있는 동안 기존 방의 슬라이드는 유지됩니다.
